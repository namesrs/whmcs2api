	...
	if($json['template'] == 'REQUEST_UPDATE')
  {
    ...
    $domainid = (int)$json['itemdetails']['custom_field'];
      ...
      if($substatus != '') $substatus_name = $json['status']['substatus'][$substatus];
  	  ...
  	  {
  	    $stm = $pdo->prepare('UPDATE tbldomains SET expirydate = :exp, nextduedate = DATE_SUB(:exp2,INTERVAL (SELECT value FROM tblconfiguration WHERE setting = "DomainSyncNextDueDateDays" ORDER BY id DESC LIMIT 1) day) WHERE registrar = "namesrs" AND id = :id');
  	    $stm->execute(array('exp' => $expire, 'exp2' => $expire, 'id' => $domainid));
        logModuleCall(
          'nameSRS',
